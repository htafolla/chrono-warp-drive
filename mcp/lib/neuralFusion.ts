...
export class NeuralFusion {
  ...
  private async processSpectrum(spectrumData: SpectrumData): Promise<number[]> {
    ...
    try {
      ...
      console.error('processSpectrum error:', err.message);
      throw err;
    }
  }
  ...
  private sampleArray(array: number[], targetSize: number): number[] {
    ...
    if (array.length < targetSize) {
      // Interpolate to reach target size
      const result: number[] = [];
      const ratio = (array.length - 1) / (targetSize - 1);
      for (let i = 0; i < targetSize; i++) {
        const srcIndex = i * ratio;
        const lower = Math.floor(srcIndex);
        const upper = Math.min(Math.ceil(srcIndex), array.length - 1);
        const t = srcIndex - lower;
        result.push(array[lower] * (1 - t) + array[upper] * t);
      }
      return result;
    }
